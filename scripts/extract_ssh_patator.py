#!/usr/bin/env python3
"""
Extract SSH-Patator attack payloads from the Tuesday CICIDS2017 capture,
using the GeneratedLabelledFlows CSV as ground truth (Label == "SSH-Patator").

Matches by (src_ip, src_port, dst_ip, dst_port) alone - no timestamp
involved, see lib/flow_labels.py for why.

Previous runs stopped at 500 by hitting an old cap, not by exhausting the
file - default cap here is 1000 to find the true ceiling.

    python scripts/extract_ssh_patator.py \\
        --pcap /path/to/Tuesday-WorkingHours.pcap \\
        --csv /path/to/Tuesday-WorkingHours.pcap_ISCX.csv \\
...
def run(pcap_path: str, csv_path: str, out_dir: str, cap: int,
    ...
    print_summary("SSH-Patator", [stats])
